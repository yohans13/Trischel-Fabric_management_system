...
<div>
    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper">
                        ...
                        <div class="card">
                                    ...
                                    <div class="alert-body">
                                        ...
                                        <p>
                                          ...
                                          <code class="highlighter-rouge">waste </code>Fabric (kg) | Contaminated fabric (kg).
                                                ...
                                                <h4 class="text-dark">{{year}} Water Chart</h4>
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        <tr v-for="water in waters" :key="water.id">
                                          
                                            <td class="text-nowrap"> {{water.subName}}</td>
                                            <td class="text-nowrap"> {{water.jan}} </td>
                                            <td class="text-nowrap">{{water.feb}} </td>
                                            <td class="text-nowrap">{{water.mar}}</td>
                                            <td class="text-nowrap">{{water.aprl}}</td>
                                            <td class="text-nowrap">{{water.may}}</td>
                                            <td class="text-nowrap">{{water.june}}</td>
                                            <td class="text-nowrap">{{water.july}}</td>
                                            <td class="text-nowrap">{{water.aug}}</td>
                                            <td class="text-nowrap">{{water.sep}}</td>
                                            <td class="text-nowrap">{{water.oct}}</td>
                                            <td class="text-nowrap">{{water.nov}}</td>
                                            <td class="text-nowrap">{{water.dec}}</td>
                                            <td class="text-nowrap">
                                            <router-link  :to="{name:'editWater' ,params:{id:water.id}}" >  
                                                             <img v-bind:src="form.editicon" alt="edit" height="18" width="18" />
                                                             
                                                        </router-link> &nbsp

                                                          <a @click="deleteWater(water.id)" > 
           ...
           <div>
            <router-link to="/newwater" class="btn btn-primary">Add Record</router-link>
          </div>
...
<script>
   ...
   export default{
                ...
                computed:{
                  filtersearch(){
                    return this.waters.filter(water=>{
                      return water.year.match(this.updatedYear)
                    })
                  },
                ...
                methods:{
                  allyears(){
                    ...
                    allWaters(){
                      let year =this.year
                    axios.get('/api/getWater/'+year)
                    .then(({data})=>(this.waters=data))
                ...
                deleteWater(id){
                    Swal.fire({
                      ...
                      if (result.isConfirmed) {
                          ...
                          axios.delete('/api/WaterDelete/'+id)
                            .then(() =>{
                                this.waters = this.waters.filter(water =>{
                                  return water.id != id
                                })
                            })
                            .catch(()=>{
                              this.$route.push({name:'water'})
                            })
                          Swal.fire(
                          'Deleted!',
